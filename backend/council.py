                ...
                try:
                    ...
                    if response is not None:
                        if response.get("error"):
                            # Error already logged by provider - just pass through
                            ...
                            # Check for empty content (status mismatch fix)
...
async def stage2_collect_rankings(
    ...
    async def _query_safe(m: str):
        try:
            ...
            return m, {"error": True, "error_message": error_message}
        ...
        if response is None or response.get("error"):
            ...
            asyncio.create_task(
                log_model_error(
                    ...
                    message=error_msg,
                    status_code=response.get("error_code") if response else None,
                )
            )
...
def calculate_aggregate_rankings(
    ...
    for ranking in stage2_results:
        ...
        parsed_ranking = parse_ranking_from_text(
            ranking_text, expected_count=expected_count
        )
